# DAG owner: data_engineer — SQLite implementation and persistence API.
"""SQLite access: users, scores, timestamps. Parameterized SQL only (security baseline)."""
...
from database.validation import validate_score, validate_username
...
def ensure_user(conn: sqlite3.Connection, username: str) -> int:
    u = get_user_by_name(conn, validate_username(username))
    if u:
        return u.id
...
def save_score(conn: sqlite3.Connection, user_id: int, score: int, *, when: datetime | None = None) -> int:
    ...
    cur = conn.execute(
        ...
        (user_id, score, when.isoformat()),
    )
    conn.commit()
    return int(cur.lastrowid)


def leaderboard_top(conn: sqlite3.Connection, limit: int = 50) -> list[LeaderboardRow]:
    rows = conn.execute(
        """
...
class Database:
    ...
    def leaderboard_top(self, limit: int = 50) -> list[LeaderboardRow]:
